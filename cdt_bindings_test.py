...
def save_triangulation_as_off(t: cdt.Triangulation, off_file) -> None:
    with open(off_file, "w") as f:
        f.write(f"OFF\n")
        f.write(f"{len(t.vertices)} {len(t.triangles)} 0\n")
        for v in t.vertices:
            f.write(f"{v.x} {v.y} 0\n")
...
def read_input_file(input_file):
    with open(input_file, "r") as f:
        n_verts, n_edges = tuple([int(s) for s in f.readline().split()])
...
def test_triangulate_input_file() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with open(off_file, 'rb') as f:
            assert hashlib.md5(f.read()).hexdigest() == '29110b24778e3e7d60bd2e99451fd6de', "Wrong OFF file contents"
